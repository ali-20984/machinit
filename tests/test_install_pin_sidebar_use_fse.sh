...
echo "Testing top-level install --pin-sidebar uses finder_sidebar_editor by default (dry-run)..."
...
if echo "$OUT" | grep -q "finder_sidebar_editor" || echo "$OUT" | grep -q "FinderSidebar().add"; then
    ...
    echo "FAIL: expected finder_sidebar_editor invocation not found in output" >&2
    echo "$OUT" >&2
    exit 1
fi
