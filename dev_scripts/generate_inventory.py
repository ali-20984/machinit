...
def main():
    ...
    functions = sorted([(n.lower(), d) for (n, d) in parse_functions(ASSETS / '.functions')])

    completions_dir = ASSETS / 'completions'
    completions = []
    if completions_dir.exists():
        completions = sorted([p.name.lstrip('_') for p in completions_dir.iterdir() if p.is_file() and p.name.startswith('_')])
    ...
    defaults = set()
    rx2 = re.compile(r"set_(?:user_)?default\s+(\S+)\s+(?:\"([^\"]+)\"|\'([^']+)\'|(\S+))")
    for f in SCRIPTS.glob('*.sh'):
        for ln in f.read_text().splitlines():
